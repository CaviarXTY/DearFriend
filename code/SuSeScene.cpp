...
Scene* SuSeScene::createScene()
{
	auto scene = Scene::create();
	auto layer = SuSeScene::create();
...
bool SuSeScene::init()
{
	...
	_tileMap2 = TMXTiledMap::create("/test/suse.tmx");
...
//游戏每一帧都会调用的方法
void SuSeScene::update(float delta) {
	//onContactBegin(_npc);
	_count++;
...
Vec2 SuSeScene::tileCoordForPosition(Vec2 position) {
...
//计算玩家将移动位置
Vec2 SuSeScene::willMove(int tag) {
...
//计算镜头位置
Vec2 SuSeScene::getPositionNow(Vec2 position) {
...
//获取Object对象坐标
Vec2 SuSeScene::getObjectByName(std::string Layername, std::string name) {
	//获取对象层
...
//对话框
void SuSeScene::showText(std::string name, std::string said, bool visble) {
	if (visble) {
		log("true");
...
//键盘响应事件
void SuSeScene::keyListener(EventKeyboard::KeyCode keycode, bool set) {
...
Animate* SuSeScene::getAnimateByName(std::string animName, float delay, int animNum) {
...
//玩家+地图动作
void SuSeScene::playerMove(int tag) {
...
//碰撞处理函数
bool SuSeScene::onContactBegin(Sprite* s) {
	//碰撞检测，矩形碰撞    
