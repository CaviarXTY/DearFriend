...
Scene* HouseScene::createScene()
{
	auto scene = Scene::create();
	auto layer = HouseScene::create();
...
bool HouseScene::init()
{
	...
	audioengine->preloadBackgroundMusic("Message From Home.mp3");
	SimpleAudioEngine::getInstance()->playBackgroundMusic("Message From Home.mp3", true);
	...
	_tileMap2 = TMXTiledMap::create("/test/home.tmx");
...
Vec2 HouseScene::tileCoordForPosition(Vec2 position) {
...
//计算玩家将移动位置
Vec2 HouseScene::willMove(int tag) {
...
//计算镜头位置
Vec2 HouseScene::getPositionNow(Vec2 position) {
...
//获取Object对象坐标
Vec2 HouseScene::getObjectByName(std::string Layername, std::string name) {
	//获取对象层
...
//对话框
void HouseScene::showText(std::string name, std::string said, bool visble) {
	if (visble) {
		log("true");
...
//键盘响应事件
void HouseScene::keyListener(EventKeyboard::KeyCode keycode, bool set) {
...
Animate* HouseScene::getAnimateByName(std::string animName, float delay, int animNum) {
...
//玩家+地图动作
void HouseScene::playerMove(int tag) {
...
//碰撞处理函数
bool HouseScene::onContactBegin(Sprite* s) {
	//碰撞检测，矩形碰撞    
